...
export function shouldRespondToTelegramMessage(event: TelegramMessageContext): boolean {
  ...
  console.log(`[shouldRespond] Checking: chatType=${message.chatType}, mode=${bot.mode}, username=${bot.username}, mentions=[${message.mentions.join(',')}], aliases=[${bot.aliases.join(',')}]`);

  if (!message.text.trim() || message.fromIsBot) {
    console.log(`[shouldRespond] false: empty text or from bot`);
    return false;
  }

  if (message.chatType === "private") {
    console.log(`[shouldRespond] true: private chat`);
    return true;
  }

  if (bot.mode === "always") {
    console.log(`[shouldRespond] true: mode=always`);
    return true;
  }

  if (message.replyToUserId && bot.userId && message.replyToUserId === bot.userId) {
    console.log(`[shouldRespond] true: reply to bot`);
    return true;
  }
  ...
  if (
    ...
    console.log(`[shouldRespond] true: reply to bot username`);
    return true;
  }

  if (bot.username && message.mentions.includes(bot.username.toLowerCase())) {
    console.log(`[shouldRespond] true: mentioned`);
    return true;
  }

  const hasAlias = includesAlias(message.text, bot.aliases);
  console.log(`[shouldRespond] ${hasAlias}: alias check`);
  return hasAlias;
}
      ...
      (async () => {
        try {
          ...
          while (!stopped) {
            ...
            try {
              log("info", "Polling for updates", { offset, personaId: config.personaId });
              ...
              for (const update of updates) {
                ...
                if (!shouldRespondToTelegramMessage(event)) {
                  log("info", "Skipping message - shouldRespondToTelegramMessage returned false", {
