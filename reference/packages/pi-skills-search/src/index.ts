...
export interface FinpalSkillLike {
  ...
  execute(
    input: PortableSearchInput & { entity: string },
...
function createProposal(entity: string, focus?: SearchFocus[]) {
...
function createBoard(entity: string, focus?: SearchFocus[], previousGaps?: string[]): PortableResearchBoard {
  return {
...
function createSearchPlan(entity: string, queries: string[], gaps?: string[]): SearchPlan {
...
function generateQueries(input: PortableSearchInput): string[] {
...
function detectGaps(data: PortableSearchData): string[] {
  const gaps: string[] = [];
...
function calculateConfidence(data: PortableSearchData): number {
  let score = 0;

  if (data.entityInfo.name) score += 0.2;
  if (data.entityInfo.type) score += 0.1;
  if (data.entityInfo.summary) score += 0.15;
  if (data.plan.steps.length >= 3) score += 0.05;
  if (data.news.length >= 5) score += 0.25;
  else if (data.news.length >= 3) score += 0.18;
  else if (data.news.length > 0) score += 0.1;
  if (data.risks.length >= 3) score += 0.15;
  else if (data.risks.length > 0) score += 0.08;
  if (data.sources.length >= 5) score += 0.15;
  else if (data.sources.length >= 3) score += 0.08;
  if (data.researchBoard.coveredGaps.length >= 3) score += 0.1;
  else if (data.researchBoard.coveredGaps.length > 0) score += 0.05;

  return Math.min(score, 1);
}

function evaluateResults(results: PortableSearchResultItem[]) {
  const accepted: PortableSearchResultItem[] = [];
...
function mergeKnownFacts(board: PortableResearchBoard, results: PortableSearchResultItem[]): void {
...
function deriveHypotheses(
  ...
  isRetry: boolean
): PortableResearchBoard["hypotheses"] {
...
function updateCoveredGaps(board: PortableResearchBoard, query: string, results: PortableSearchResultItem[]): void {
...
function criticReview(query: string, results: PortableSearchResultItem[], board: PortableResearchBoard): SearchIteration {
...
function shouldStopSearching(data: PortableSearchData): string | null {
  ...
  const lowValueIterations = data.iterations.filter(
    (iteration) => !iteration.accepted || iteration.informationDelta < 0.3
  ).length;
...
function pickType(entity: string, summary?: string): string | undefined {
...
export async function runPortableDeepSearch(
  input: PortableSearchInput,
  runtime: SearchSkillRuntime
): Promise<PortableSearchData> {
  ...
  const acceptedResults: PortableSearchResultItem[] = [];
  const sources = new Set<string>();
  const news = new Map<string, PortableSearchData["news"][number]>();
  const risks = new Set<string>();
  const iterations: SearchIteration[] = [];

  let data: PortableSearchData = {
    entityInfo: {
      name: input.entity
    },
    news: [],
...
function extractEntityFromTask(task: AgentTask): string {
  return task.goal.replace(/^.*?(搜索|研究|分析|查一下)?\s*/u, "").trim() || task.goal;
}

export function createCyberBowieSearchSkill(runtime: SearchSkillRuntime): AgentSkill {
  return {
    metadata: METADATA,
    async execute(task: AgentTask): Promise<SkillResult> {
...
export function createFinpalCompatibleSearchSkill(runtime: SearchSkillRuntime): FinpalSkillLike {
  return {
    metadata: METADATA,
    async execute(input: PortableSearchInput) {
...
export function createMcpSearchRuntime(options: McpSearchRuntimeOptions): SearchSkillRuntime {
  ...
  return {
    async search(query: string): Promise<PortableSearchResultItem[]> {
      ...
      const resultItems = Array.isArray(parsed)
        ? parsed
        : typeof parsed === "object" && parsed && "pages" in parsed && Array.isArray((parsed as { pages?: unknown[] }).pages)
          ? (parsed as { pages: Array<Record<string, unknown>> }).pages
          : [];
